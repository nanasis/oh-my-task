...
export function discoverWorkspaceTasks(tasks, cwd, options = {}) {
...
export function buildResumeContextBundle(task, currentAgent, sessionLimit = 5, previousAgent) {
...
export function sessionsFromTask(task) {
...
function extractRawSection(body, heading) {
    const escaped = heading.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return new RegExp(`^## ${escaped}\\s*\\n([\\s\\S]*?)(?=^## |(?![\\s\\S]))`, "m").exec(body)?.[1]?.trim() ?? "";
}
function extractLatestCheckpoint(body) {
...
//# sourceMappingURL=resume-context.js.map